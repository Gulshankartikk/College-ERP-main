          ...
          <div className="px-10 py-5 font-oswald">
            <div className="flex flex-wrap gap-1 lg:gap-10 min-h-[50vh] items-center justify-center">
              {courses.map((course, index) => (
                ...
                >
                  <div className="h-full w-full font-semibold text-lg border-2 rounded-lg border-black bg-gray-100 flex flex-col justify-center items-center hover:scale-105 duration-150 p-3">
                    <p className="overflow-hidden text-center font-bold">{course.courseName}</p>
                    <p className="text-sm text-gray-600">({course.courseCode})</p>
                    <p className="text-xs text-gray-500">{course.duration} - {course.totalSemesters} Semesters</p>
                    <p className="text-xs text-blue-600">{course.courseType}</p>
                  </div>
                </Link>
              ))}
            </div>
